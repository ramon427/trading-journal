...
export function EditSetupCategoryPage({ categoryId }: { categoryId: string }) {
  ...
  useEffect(() => {
    if (!isNew) {
      ...
      if (category) {
        ...
        navigate('/customize?tab=setup-categories');
      }
    }
  ...
  const handleSave = () => {
    ...
    if (isNew) {
      ...
      saveSetupCategories([...categories, newCategory]);
  ...
  const handleDelete = () => {
    if (confirm('Are you sure you want to delete this category? Setups in this category will become uncategorized.')) {
      const categories = getSetupCategories();
      const filtered = categories.filter(c => c.id !== categoryId);
      saveSetupCategories(filtered);
        ...
        <div className="max-w-4xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button
                onClick={() => navigate('/customize?tab=setup-categories')}
                variant="ghost"
                size="sm"
                className="gap-2"
              >
              ...
              <div>
                <h1 className="text-2xl">{isNew ? 'Create Setup Category' : 'Edit Setup Category'}</h1>
                <p className="text-sm text-muted-foreground">
                  {isNew ? 'Create a new category for organizing setups' : 'Update the category'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => navigate('/customize?tab=setup-categories')} variant="outline" size="sm">
                Cancel
              </Button>
